...
%
% Solve this question in SOR method.
...
% Jacobian matrix function : (L+1/w*D) * Tx_u = b - (U+(1-1/w)*D) * Tx
% u : time step number
% w: pivoting parameter w~[1,7,1.8]
w = 1.7; 
P = L + (1/w)*D;
Q = U + (1-1/w)*D;
p = P^-1;
...
surfc(Y,X,T)
title(['SOR Method Heat Equation with:', ' Nx=', num2str(Nx), ', Ny=', num2str(Ny)])
xlabel('Nx-grid');
ylabel('Ny-grid');
zlabel('Temperature');
colorbar
